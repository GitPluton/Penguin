...
async def _apply(ctx,app = None): #there was no app
  x = await db(ctx)
  try:
    x["q"]
    if not x["q"]:
...
#Start of multi applications code
  ...
  for question in appkey: #this was x["q"]
    n += 1
